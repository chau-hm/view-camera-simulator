...
- Scope: migrate production `focus-targets-sharp` pass/fail and criterion score
  from legacy wedge `sharpness` to physical worst-sample patch sharpness, and
  keep Architecture + Foreground compound technique requirements explicit.
...
- Architecture + Foreground DOF/Compound requires physical patch sharpness of at
  least `0.5`, matching the shared Acceptable presentation boundary. Compound
  separately requires Tilt use/range and the calibrated `100 mm` Focus move.
...
| AF DOF / focus targets | foreground-near, foreground-middle, building-base, building-middle | 0.6 → 0.5 | 0.634 | 0.604 / 0.039606 | 0.000 / 0.115217 | 0.000 / 0.115217 |
| AF Compound / focus targets | foreground-near, foreground-middle, building-base, building-middle | 0.6 → 0.5 | 0.634 | 0.604 / 0.039606 | 0.000 / 0.223193 | 0.212 / 0.078830 |
...
0.5`. At the canonical public Tilt + Focus state, the measured target values
are:

| Aperture | foreground-near | foreground-middle | building-base | building-middle | Worst physical score / CoC | Task result |
| --- | --- | --- | --- | --- | ---: | --- |
| f/11 | .902 Sharp / .009848 mm | .142 Soft / .085819 mm | .000 Soft / .115217 mm | .839 Sharp / .016108 mm | .000 / .115217 mm | fail |
| f/22 | .951 Sharp / .004924 mm | .571 Acceptable / .042910 mm | .424 Soft / .057609 mm | .919 Sharp / .008054 mm | .424 / .057609 mm | fail |
| f/32 | .966 Sharp / .003385 mm | .705 Acceptable / .029500 mm | .604 Acceptable / .039606 mm | .945 Sharp / .005537 mm | .604 / .039606 mm | pass |

The shared presentation policy makes f/22 ineligible because `building-base`
is Soft. f/32 is the first public aperture with every required target
Acceptable or Sharp and is now the sole `dofPassingApertures` value. The exact
compound bypass `Rise 20 / Tilt 0 / Focus 6200 / f/22` has physical focus score
`.414` but fails the explicit Tilt-used and Tilt-range criteria (and the f/22
focus threshold); the same no-Tilt state at f/32 passes physical focus but
still fails those movement criteria. A no-Focus state at
`Rise 20 / Tilt 2 / Focus 9490 / f/32` has physical focus score `.672` and
fails only the calibrated Focus-used criterion. No scene calibration or
canonical camera state was changed.

## Validation

- Focused physical task/readout and all affected task/optics tests: pass.
- Full unit/integration suite: 142 files, 1,341 tests passed.
- Typecheck, lint, CSS check, and build: pass.
- Serial bounded Chromium validation: 8 passed across the affected
  Architecture + Foreground DOF, Compound, Guided Lesson, and Tilt + Focus
  flows, including f/22 rejection, f/32 completion, and the no-Tilt bypass.
- `npm run ci:local:e2e`: CSS/lint/typecheck/unit/build and all affected
  Architecture + Foreground E2E files passed; stopped at the known unrelated
  Focus Fundamentals baseline in `focus-fundamentals-selectable-focus.spec.ts:155`
  because RTT `ownerId/resourceGeneration` diagnostics are absent. Its
  responsive companion passed.
