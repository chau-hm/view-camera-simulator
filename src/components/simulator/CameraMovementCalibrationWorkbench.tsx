...
const NumberField = ({ label, value, onChange, step = 1, min, max, disabled = false }: { label: string; value: number; onChange: (value: number) => void; step?: number; min?: number; max?: number; disabled?: boolean }) => (
  <label className="camera-calibration-workbench__field">{label}
    <input type="number" value={value} step={step} min={min} max={max} disabled={disabled} onChange={(event) => onChange(Number(event.target.value))} />
  </label>
);
...
export const CameraMovementCalibrationWorkbench = ({
  ...
  const updateSection = (next: CameraMovementCalibrationOverrides) => update({
  ...
  return (
    <section className="camera-calibration-workbench" aria-label="Camera movement calibration workbench">
      ...
      <fieldset><legend>Subject</legend><div className="camera-calibration-workbench__grid"><NumberField label="Columns" min={1} max={8} value={calibration.subject.columns} onChange={(value) => updateSection({ geometry: { columns: value } })} /><NumberField label="Rows" min={1} max={8} value={calibration.subject.rows} onChange={(value) => updateSection({ geometry: { rows: value } })} /><NumberField label="Levels" min={3} max={12} value={calibration.subject.levels} onChange={(value) => updateSection({ geometry: { levels: value } })} /><NumberField label="Cube (mm)" min={50} max={1000} value={calibration.subject.cubeSizeMm} onChange={(value) => updateSection({ geometry: { cubeSizeMm: value } })} /><NumberField label="Horizontal gap (mm)" min={0} max={1000} value={calibration.subject.horizontalGapMm} onChange={(value) => updateSection({ geometry: { horizontalGapMm: value } })} /><NumberField label="Vertical gap (mm)" min={0} max={1000} value={calibration.subject.verticalGapMm} onChange={(value) => updateSection({ geometry: { verticalGapMm: value } })} /><NumberField label="Subject distance (mm)" min={500} max={10000} value={calibration.subject.originWorld.z} onChange={(value) => updateSection({ geometry: { subjectDistanceMm: value } })} /></div></fieldset>
      <fieldset><legend>Optics</legend><div className="camera-calibration-workbench__grid"><NumberField label="Focal length (mm)" min={1} max={1000} value={calibration.optics.provisionalFocalLengthMm} onChange={(value) => updateSection({ optics: { provisionalFocalLengthMm: value, focalLengthCandidatesMm: Array.from(new Set([...calibration.optics.focalLengthCandidatesMm, value])) } })} /><NumberField label="Focus distance (mm)" min={2} max={10000} value={calibration.optics.provisionalFocusDistanceMm} onChange={(value) => updateSection({ optics: { provisionalFocusDistanceMm: value } })} /></div></fieldset>
      <fieldset><legend>Rig</legend><div className="camera-calibration-workbench__grid"><NumberField label="Arc angle (°)" min={0.1} max={45} value={calibration.cameraRig.highArcAngleDeg} onChange={(value) => updateSection({ rig: { arcAngleDeg: value } })} /><NumberField label="Base pitch (°)" min={-20} max={20} value={calibration.cameraRig.provisionalBasePitchDeg} onChange={(value) => updateSection({ rig: { provisionalBasePitchDeg: value } })} /></div></fieldset>
      <fieldset><legend>Movements</legend><label className="camera-calibration-workbench__field">Viewpoint anchor<select value={camera.viewpointAnchor} onChange={(event) => setAnchor(event.target.value as "mid" | "high" | "low")}><option value="mid">Mid</option><option value="high">High</option><option value="low">Low</option></select></label><div className="camera-calibration-workbench__grid"><NumberField label="Front rise (mm)" min={CAMERA_CONSTANTS.riseMinMm} max={CAMERA_CONSTANTS.riseMaxMm} disabled={camera.viewpointAnchor !== "mid"} value={camera.frontRiseMm} onChange={setRise} /><NumberField label="Front tilt (°)" min={CAMERA_CONSTANTS.tiltMinDeg} max={CAMERA_CONSTANTS.tiltMaxDeg} disabled={camera.viewpointAnchor !== "mid"} value={camera.frontTiltDeg} onChange={setTilt} /><NumberField label="Front swing (°)" min={CAMERA_CONSTANTS.swingMinDeg} max={CAMERA_CONSTANTS.swingMaxDeg} disabled={camera.viewpointAnchor !== "mid"} value={camera.frontSwingDeg} onChange={setSwing} /><NumberField label="Rear rise (mm)" min={CAMERA_CONSTANTS.riseMinMm} max={CAMERA_CONSTANTS.riseMaxMm} disabled={camera.viewpointAnchor !== "mid"} value={camera.rearRiseMm} onChange={setRearRise} /><NumberField label="Rear tilt (°)" min={CAMERA_CONSTANTS.tiltMinDeg} max={CAMERA_CONSTANTS.tiltMaxDeg} disabled={camera.viewpointAnchor !== "mid"} value={camera.rearTiltDeg} onChange={setRearTilt} /><NumberField label="Camera body pitch (°)" min={-45} max={45} value={camera.cameraBodyPitchDeg} onChange={setPitch} /></div>{camera.viewpointAnchor !== "mid" && <p role="note">Standard movements are disabled at high/low anchors; camera body pitch remains enabled.</p>}</fieldset>
      ...
      <fieldset><legend>Presentation</legend><div className="camera-calibration-workbench__grid"><NumberField label="Outer vertical weight" min={0.1} max={10} step={0.1} value={calibration.presentation.outerVerticalWeight} onChange={(value) => updateSection({ presentation: { outerVerticalWeight: value } })} /><NumberField label="Outer horizontal weight" min={0.1} max={10} step={0.1} value={calibration.presentation.outerHorizontalWeight} onChange={(value) => updateSection({ presentation: { outerHorizontalWeight: value } })} /><NumberField label="Internal edge weight" min={0.1} max={10} step={0.1} value={calibration.presentation.internalEdgeWeight} onChange={(value) => updateSection({ presentation: { internalEdgeWeight: value } })} /><NumberField label="Internal edge opacity" min={0.01} max={1} step={0.01} value={calibration.presentation.internalEdgeOpacity} onChange={(value) => updateSection({ presentation: { internalEdgeOpacity: value } })} /></div></fieldset>
      {diagnostics ? (
        <fieldset>
