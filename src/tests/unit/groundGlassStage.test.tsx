...
describe("GroundGlassStage pointer and keyboard interactions", () => {
  ...
  it("click toggles zoom when not dragged", () => {
  ...
  it("zoomed click toggles zoom out when movement below threshold", () => {
    const onToggle = vi.fn();
    ...
    fireEvent.pointerUp(stage, { pointerId: 2, button: 0, clientX: 121, clientY: 82 }); // small movement

    expect(onToggle).toHaveBeenCalledTimes(1);
  });

  it("dragging while zoomed pans and does not toggle zoom", () => {
    const onToggle = vi.fn();
    ...
    // pointer down
    fireEvent.pointerDown(stage, { pointerId: 3, button: 0, clientX: 50, clientY: 50 });
    // move beyond threshold
    fireEvent.pointerMove(stage, { pointerId: 3, button: 0, clientX: 80, clientY: 90 });
    // up
    fireEvent.pointerUp(stage, { pointerId: 3, button: 0, clientX: 80, clientY: 90 });
  ...
  it("tiny movement is treated as click", () => {
    ...
    fireEvent.pointerDown(stage, { pointerId: 4, button: 0, clientX: 200, clientY: 200 });
    fireEvent.pointerMove(stage, { pointerId: 4, button: 0, clientX: 202, clientY: 201 }); // below threshold
    fireEvent.pointerUp(stage, { pointerId: 4, button: 0, clientX: 202, clientY: 201 });
  ...
  it("non-primary pointer does nothing", () => {
    ...
    fireEvent.pointerDown(stage, { pointerId: 5, button: 2, clientX: 10, clientY: 10 });
    fireEvent.pointerUp(stage, { pointerId: 5, button: 2, clientX: 10, clientY: 10 });
  ...
  it("pointer cancel clears state and does not toggle", () => {
    ...
    fireEvent.pointerDown(stage, { pointerId: 6, button: 0, clientX: 60, clientY: 60 });
    fireEvent.pointerCancel(stage, { pointerId: 6 });
    fireEvent.pointerUp(stage, { pointerId: 6, button: 0, clientX: 60, clientY: 60 });
