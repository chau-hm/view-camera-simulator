...
describe("Ground Glass RTT scene registration", () => {
  ...
  it("includes Macro Scene 2 in the centralized RTT scene set", () => {
    expect(RTT_SCENES).toContain("macro-depth-of-field");
    expect(isGroundGlassRttScene("macro-depth-of-field")).toBe(true);
  });

  it("includes Macro Scene 3 in the centralized RTT scene set", () => {
    expect(RTT_SCENES).toContain("macro-oblique-plane");
