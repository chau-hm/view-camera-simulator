export const simulatorMessages = {
  ...
  controls: {
    ...
    lensCopy:
      "Same camera position. Changing focal length changes angle of view and framing, not perspective; perspective changes when the camera position changes. Lens coverage is separate and determines how far the standards can move before the film reaches the projected image edge.",
    lensWide: "Wide",
    lensStandard: "Standard",
    lensOptionLabel: "{{focalLength}} mm — {{label}} — {{coverage}}",
    lensCoverageLabel: "Coverage",
    lensCoverageNotModelled: "Not modelled",
    lensSimulatorCoverage: "{{angle}}° simulator coverage",
    ...
    macroBellowsExtension: {
      ...
      stages: {
        early: {
          ...
          coverage: "Lens-to-film distance ({{extension}}) is already beyond its infinity-focus reference. Closer focus moves the rear standard and film plane farther from the lens, while the 4×5 film rectangle keeps the same physical dimensions. The current fixed angular-coverage profile projects an Image Circle diameter of {{imageCircle}}, larger than its infinity-reference footprint.",
        },
        intermediate: {
