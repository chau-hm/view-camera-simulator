...
export const groundGlassFootprintAxesToRttPixels = (input: {
  ...
  if (!values.every(Number.isFinite) ||
      ...
      input.displayBlurScale <= 0) {
    return { majorAxisPx: [0, 0], minorAxisPx: [0, 0] };
  }

  const cos = Math.cos(input.orientationRad);
  const sin = Math.sin(input.orientationRad);
  const scaleX = input.renderWidthPx / input.filmWidthMm * input.displayBlurScale;
  const scaleY = input.renderHeightPx / input.filmHeightMm * input.displayBlurScale;
